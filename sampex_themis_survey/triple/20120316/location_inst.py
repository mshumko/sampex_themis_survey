...
during  the 2012-03-16 triple conjunction.
"""
...
themis_asi_locations = ['SNKQ', 'KAPU'] # SNKQ or KAPU
themis_probes = ['A'] # Also try 'A', 'E', 'D'
alt = 110  # km
times = [
    datetime(2012, 3, 16, 9, 20),
    datetime(2012, 3, 16, 9, 31, 48),
    datetime(2012, 3, 16, 9, 40),
    datetime(2012, 3, 16, 9, 50)
    ]
plot_range = (datetime(2012, 3, 16, 9), datetime(2012, 3, 16, 10))
lat_bounds=(44, 66)
lon_bounds=(-91, -66)
...
spec = gridspec.GridSpec(
    nrows=len(themis_asi_locations)+1+3, 
    ncols=n, figure=fig, height_ratios=(2, 1, 1, 1, 1, 1)
    )
...
cx = [fig.add_subplot(spec[i, :], sharex=bx) 
    for i in range(len(themis_asi_locations)+1, len(themis_asi_locations)+4)]
for cx_i in cx[:-1]:
